...
pub(crate) struct PurrPounceInput  {
...
impl Parse for PurrPounceInput {
...
// The macro is similar to the purr_tentacle macro
// But specifically adds a sub-manager to the main manager's primary queue
pub fn purr_pounce_impl(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let input = syn::parse_macro_input!(input as PurrPounceInput);
    ...
    let body = match (&input.duplicate_policy, &input.insert_position) {
        (Some((_, _, dup)), Some((_, _, ins))) => quote::quote! {
            #manager_name.insert(#stage_enum::PurrChain(sub_manager_index), #dup, #ins);
        },
        (Some((_, _, dup)), None) => quote::quote! {
            #manager_name.push(#stage_enum::PurrChain(sub_manager_index), #dup);
        },
        (None, None) => quote::quote! {
            #manager_name.push_and_delete(#stage_enum::PurrChain(sub_manager_index));
        },
        _ => unreachable!(), 
    };

    let expanded = quote::quote! {
        if let #stage_enum::PurrChain(sub_manager_index) = #sub_cat_manager_procces {
            if let Some(sub_manager_box) = #manager_name.get_sub_manager_mut(sub_manager_index) {
                #body;

                #manager_name.set_condition(
                    #stage_enum::PurrChain(sub_manager_index),

                    // Setting the flag condition
                    // Which will be executed later in the update when the sub-manager queue is empty
                    purrgress::cat_stage_manager::condition::PurrFlag::new()
                );
            };
        };
    };
