...
pub fn purr_rumble_impl(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let queue_check_condition  = match &input.rumble_policy {
        Some((_, _, rumble)) => quote::quote! {
            (#rumble != purrgress::types::RumblePolicy::Parallel 
                ...
                purrgress::types::PurrEvent::Idle => {
                    if #manager_name.current_vec_query().contains(&#sub_cat_manager_procces) {
                        if let Some(sub_manager_flag) = #manager_name
                            .get_condition_mut::<purrgress::condition::PurrFlag>(#sub_cat_manager_procces) {
                    ...
                    Some(purrgress::types::PurrEvent::Idle)
                },
                purrgress::types::PurrEvent::Running(run_stage) => {
                    Some(purrgress::types::PurrEvent::Running(run_stage))
                },
                purrgress::types::PurrEvent::Transition { from, to } => {
                    Some(purrgress::types::PurrEvent::Transition { from, to })
                },
            }
        }
    };
